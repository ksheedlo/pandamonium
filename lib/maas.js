...
if (config.IDENTITY_ENDPOINT) {
  clientOpts.identityEndpoint = config.IDENTITY_ENDPOINT;
}

client = ksks(clientOpts);
...
function requestOne(props, cb) {
  ...
  if (config.TENANT_ID) {
    authOpts.tenantId = config.TENANT_ID;
  }

  client.authenticate(authOpts, function (err, catalog) {
